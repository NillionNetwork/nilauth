...
mod tests {
    ...
    async fn cost(#[case] blind_module: BlindModule, #[case] cost_unils: u64) {
        ...
        token_price_service.expect_nil_token_price().return_once(move || Ok(50.into()));
        let costs = BlindModuleCosts { nildb: 100.into(), nilai: 150.into() };
        let service = DefaultSubscriptionCostService::new(Arc::new(token_price_service), costs);
        let cost = service.blind_module_cost(blind_module).await.expect("failed to get cost");
