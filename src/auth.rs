    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        ...
        let payload = match parts.headers.get(AUTHORIZATION_HEADER) {
            Some(value) => value.to_str().map_err(|_| make_unauthorized("header value is not valid utf8"))?,
            None => {
                return Err(make_unauthorized(format!("`{AUTHORIZATION_HEADER}` header missing")));
            }
        };
        let payload =
            payload.strip_prefix("Bearer ").ok_or_else(|| make_unauthorized("missing `Bearer ` prefix in header"))?;
        let token =
            NucTokenEnvelope::decode(payload).map_err(|e| make_unauthorized(format!("malformed token: {e}")))?;
...
mod tests {
    ...
    impl NucAuthBuilder {
        async fn build(self, header: String) -> Result<NucAuth, (StatusCode, Json<RequestHandlerError>)> {
    ...
    impl Default for NucAuthBuilder {
        fn default() -> Self {
            NucAuthBuilder { validator: NucValidator::new(&[]), nilauth_did: Did::key(random_public_key()) }
        }
    }
